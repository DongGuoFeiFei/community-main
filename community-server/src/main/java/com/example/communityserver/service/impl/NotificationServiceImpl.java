...
//                    vo.setCreatedAt(notification.getCreatedAt());
    ...
    private IPage<NotificationListVo> getFollowNotificationsVo(GetNotificationsParam param) {
        ...
        LambdaQueryWrapper<Notification> notificationQueryWrapper = new LambdaQueryWrapper<>();
        notificationQueryWrapper
                .in(Notification::getContentId, followIds)
                .eq(Notification::getUserId, SecurityUtils.getLoginUserId())
        ...
        //转换为VO列表
        List<NotificationListVo> listVos = followIPage
                .getRecords()
                .stream()
                .map(follow -> {
                    NotificationListVo vo = new NotificationListVo();
                    ...
                    // 设置发送者信息
                    senders.stream()
                            .filter(sender -> sender.getUserId().equals(follow.getFollowingId()))
                            .findFirst()
                            .ifPresent(sender -> {
                                vo.setSenderId(sender.getUserId());
                                vo.setSenderName(sender.getNickname());
                    ...
                    notificationList
                            .stream()
                            .filter(notification -> follow.getId().equals(notification.getContentId()))
                            .findFirst()
    ...
    private IPage<NotificationListVo> getFavoriteArticleNotificationsVo(GetNotificationsParam param) {
        ...
        List<Long> articleIds = userFavorites.stream()
                .filter(userFavorite -> userFavorite.getType().equals(ActiveTypeEnum.FAVORITE_ARTICLE))
        ...
        List<NotificationListVo> listVos = notificationIPage
                ...
                .map(notification -> {
                    ...
                    userFavorites
                            .stream()
                            .filter(userFavorite -> userFavorite.getFavoriteId().equals(notification.getContentId()))
                            .findFirst()
                            .ifPresent(userFavorite -> {
                                articles.stream()
                                        .filter(article -> article.getArticleId().equals(userFavorite.getTargetId()))
                                        .findFirst()
                                        .ifPresent(article -> {
                                            vo.setSourceId(article.getArticleId());
                                            vo.setSourceTitle(article.getTitle());
